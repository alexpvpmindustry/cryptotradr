...
from aver5_trader import get_current_price,market_trade,price_action_signal, read_signal,log_trade_results
import json
...
trade_param = trade_params["ver5"]["params"][param_choice]
        ...
        while status=="HOLD": 
            ...
            if loopcounts%6==0 and not stdmean_status_exited:# read exit status (remove the False and for effect)
                ...
                if stdmean_status == "EXIT": #checks if we are exiting becos of signal
                    ...
                    strr = f"Exit🪃Signal {symbol}{interval} {sign}pc{param_choice}{emoji} `{cur_price:{price_format}}` "
