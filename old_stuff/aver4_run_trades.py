...
from aver4_trader import write_signal
from aver4_funcs import get_data,get_entrys_exits,get_entry_signals, validate_dfmpl
...
def get_signal(firstRun=False):
    ...
    if new_entry and entered:
        ...
        cmd = ["python","aver4_master_trades.py",f"{param_choice}","15",f"{enterdftime}","TEST",f"{xx:.6f}"]
        cmd = " ".join(cmd)
...
if "m" in interval:
    ...
    time.sleep(delay)
    schedule.every(intvl).minutes.at(":02").do(get_signal_with_warnings)
else:
